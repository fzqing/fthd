...
0x58684c11,0xc1611dab,0xb6662d3d,0x076dc4190,0x1db7106,
...
//Generate the CRC32 table for search
...
int Mu_Crc32File(int fd, unsigned int crc32){
	...
	while((bytes_read=read(fd,buf,MAX_CRC_LEN))>0){
		...
		for(i=0;i<bytes_read;i++){
			...
			crc = ((crc>>8) & 0x00FFFFFF) ^ CRC32_Tab[(crc^buf[i])&0xFF ];
...
int Mu_Crc32Segment(int fd,off_t seek,size_t char_length,unsigned int crc32){
	...
	while((bytes_read = read(fd,buf,MAX_CRC_LEN))>0){
		...
		//Keep the char_length for check
		...
		//Stop while loop when 'char_length' matched
